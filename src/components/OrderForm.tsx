...
import { LOCATION_DATA } from "@/config/constants"; // <-- Import here
...
export const OrderForm = ({ availableLocations, onSubmit }: OrderFormProps) => {
  ...
  // Use the first default location from our constants as the initial state
  const [selectedLocation, setSelectedLocation] = useState(
    LOCATION_DATA.DEFAULTS[0],
  );
  ...
  const handleLocationChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    // Check against the constant instead of a hardcoded string
    if (e.target.value === LOCATION_DATA.ADD_NEW_KEY) {
  ...
  return (
    <form
      ...
      className="mb-8 flex flex-col gap-4 bg-gray-50/50 p-4 rounded-xl border border-gray-100"
    >
        ...
        <div className="w-full sm:w-1/2 relative">
          {isCustomLocation ? (
            ...
            >
              ...
              <option value={LOCATION_DATA.ADD_NEW_KEY}>
                + Add new location...
              </option>
            </select>
          )}
